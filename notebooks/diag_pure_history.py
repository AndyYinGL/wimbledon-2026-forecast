"""Final isolation: split features into PURE-HISTORY (past serve results only,
no current score/position) vs POSITION (total_games, faces_bp -- functions of
the current within-game/within-set score).

If the gain lives only in POSITION features, it is about where we are in the
match (situational / possible position leakage), not time momentum. If
PURE-HISTORY features gain on their own, that is genuine momentum.
game_lead excluded. Train 2011-2023, test 2024.
"""
import sys
...
def run(tr, te, feats, yte, ll1, label):
    ...
    print(f"  {label:38s} test {ll:.5f}  impr {ll1-ll:+.6f}")
...
def main():
    ...
    pure_history = ["f_logit_pserve", "prev_serve_won", "serve_streak", "prev_game_held"]
    position = ["f_logit_pserve", "total_games", "server_faces_bp"]
    both = ["f_logit_pserve", "prev_serve_won", "serve_streak", "prev_game_held",
            "total_games", "server_faces_bp"]
    run(tr, te, pure_history, yte, ll1, "PURE HISTORY (momentum only)")
    run(tr, te, position, yte, ll1, "POSITION (total_games + faces_bp)")
    run(tr, te, both, yte, ll1, "both")
